...
export default async function handler(req) {
  ...
  // allowlist for order
  const orderBy = ['created_at', 'total', 'weight'].includes(url.searchParams.get('orderBy'))
  ...
  let params = [];
  if (q) {
    where.push(`(crop ILIKE $1 OR growth_choice ILIKE $1 OR temp_choice ILIKE $1)`);
    params.push(`%${q}%`);
  }
  ...
  return new Response(JSON.stringify({ page, limit, count: rows.length, rows }), {
